		...
		<div class="row">
			<div class="col">
							...
							<tr>
								...
								<td>
									<form class="form-inline" method="post"
										...
										<input type="hidden" name="idIngrediente" value="${ingrediente.id}" /> <input
											type="hidden" name="idReceta" value="${receta.id}" />

										<label class="sr-only" for="inlineFormInputName2">Cantidad</label>
										<input type="text" class="form-control mb-2 mr-2" id="cantidadAlimento"
											name="cantidadAlimento" value="${ingrediente.cantidad}"
										...
										<a class="btn btn-lg btn-outline-danger mb-2"
											href="${pageContext.request.contextPath}/ingrediente/eliminar?id=${ingrediente.id}&idReceta=${receta.id}">
				...
				<form action="${pageContext.request.contextPath}/receta/anadir/ingrediente" method="post">
					<input type="hidden" name="idReceta" value="${receta.id}" /> <select name="alimento">
						<option value=""></option>
						<c:forEach items="${alimentos}" var="alimento">
							<option value="${alimento.id}">${alimento.nombre}</option>
						</c:forEach>
					</select> <input type="number" name="cantidad" value="0" /> <input type="submit"
						value="Anadir ingrediente a receta" name="guardar" />
				</form>
